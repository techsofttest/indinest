...
export default function CartPage() {
    ...
    const loadCart = () => {
      ...
      if (stored) {
        setCartItems((prevItems) => {
          if (JSON.stringify(prevItems) === stored) {
            return prevItems;
          }
  ...
  // Sync cart item count to header via custom storage event
  useEffect(() => {
    const totalCount = cartItems.reduce((acc, item) => acc + item.quantity, 0);
    localStorage.setItem("cartItemCount", String(totalCount));
    // Trigger custom events so header updates immediately
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3 text-sm text-[#010526]/80 font-sans">
                        {!["S", "M", "L", "XL", "XXL"].includes(item.size) ? (
                          <span>Size: <strong className="text-[#010526] font-bold">{item.size}</strong></span>
                        ) : (
